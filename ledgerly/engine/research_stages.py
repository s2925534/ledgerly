...
def stages_ics(workspace: Path) -> str:
    ...
    lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Ledgerly//Research Stages//EN", "CALSCALE:GREGORIAN"]
    for stage in list_stages(workspace):
        ...
        lines.extend(
            [
                "BEGIN:VEVENT",
                f"UID:{workspace_slug}-{stage_id}@ledgerly.local",
                f"DTSTAMP:{stamp}",
                f"DTSTART;VALUE=DATE:{dtstart}",
                f"SUMMARY:{_ics_escape(name)}",
                f"DESCRIPTION:{_ics_escape(f'Ledgerly research stage {stage_id} ({status})')}",
                "END:VEVENT",
            ]
        )
