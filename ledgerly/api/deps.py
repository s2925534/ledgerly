...
from ledgerly.api.auth import SESSION_COOKIE_NAME, auth_configured, extract_token, session_is_valid
...
def resolve_workspace_path(workspace: str) -> Path:
    ...
    LEDGERLY_WORKSPACE_ROOT sandbox as the `resolve_workspace` FastAPI
    ...
    When LEDGERLY_WORKSPACE_ROOT is set (a deployed instance pointed at a
    ...
    root = os.environ.get("LEDGERLY_WORKSPACE_ROOT")
    ...
    if root:
        ...
        if candidate != root_path and root_path not in candidate.parents:
            raise ApiError(
                ...
                f"Workspace must be inside LEDGERLY_WORKSPACE_ROOT ({root_path}): {workspace}",
                status_code=400,
            )
        path = candidate
    else:
...
def resolve_workspace(
    workspace: str = Query(
        ...,
        description=(
            "Absolute local workspace path, or a path relative to LEDGERLY_WORKSPACE_ROOT "
...
def require_session(
    ...
    Fails closed (503) when no LEDGERLY_API_PASSWORD is configured, rather
    than silently allowing unauthenticated access.
    """
    if not auth_configured():
        raise ApiError(
            "auth_not_configured",
            "LEDGERLY_API_PASSWORD is not set. Configure it before using this API.",
            status_code=503,
        )
