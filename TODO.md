...
- [ ] **API** - Add an overlay review view for proposed cross-reference links and suggested renamed filenames, requiring explicit per-item approval before artefacts are cross-referenced into methodology or other artefact documents. Backend is now ready for the cross-reference half: `GET /api/v1/artefacts/cross-reference` (candidates), `POST .../cross-reference/candidate-review` (set one candidate's review_status without hand-editing YAML — the missing piece for a browser-based reviewer), and `POST .../cross-reference/apply` all exist and are tested. Citation plans (`POST /api/v1/citations/plan`) have the identical hand-edit-then-apply limitation and have not had the equivalent review route added yet. Remaining work here is purely the overlay UI itself once Phase 10 picks a framework.
