...
- [x] **Done** - **API** - Add configurable batch-upload limits (`RESEARCHBOSS_UPLOAD_MAX_FILES` default 25, `RESEARCHBOSS_UPLOAD_MAX_FILE_SIZE_MB` default 50, allowed extensions reused from `sources.ALLOWED_EXTENSIONS`) with a `400 upload_batch_too_large` rejection for the whole batch when the file-count limit is exceeded, rather than silently truncating it.
...
- [x] **Done** - **API** - a desktop, web, and mobile UI strategy around local workspaces, validation reports, citation review plans, document versions, and guideline management. Decision: FastAPI + Jinja2 server-rendered shell + vanilla JavaScript, no build toolchain, no third-party JS/CSS dependency (no React/Vue/Svelte/Flutter, no Node/npm, no CDN assets) — see `researchboss/web/`. The web UI is a pure client of the already-tested `/api/v1/*` JSON API (session-cookie auth, same as any other client); it has zero engine access of its own. "Desktop"/"mobile" beyond a responsive web page are not separately addressed — out of scope until there's a concrete need, consistent with not designing for hypothetical requirements. `docs/PACKAGING.md`'s conditional Flutter desktop-sidecar section is now moot and should be read as historical.
- [x] **Done** - **API** - Define a clear API contract in `docs/api/CONTRACT.md`.
- [x] **Done** - **API** - a frontend folder or explicit planned UI structure once the backend boundary is stable. `researchboss/web/{app.py,templates/,static/}`, mounted onto the same FastAPI app `researchboss serve` already runs (`mount_web()` in `api/app.py`) — no separate process, port, or deployment step.
- [x] **Done** - **API** - UI architecture guidance that keeps UI logic out of the core engine and routes all workspace mutations through tested engine/API contracts. Enforced structurally, not just by convention: `researchboss/web/app.py` imports only from `researchboss.api.auth` (session-cookie helpers) and Jinja2/Starlette — it has no import path to `researchboss.engine` at all. Every data operation in `static/app.js` is a `fetch()` call to an existing, tested `/api/v1/*` route.
...
- [x] **Done** - **API** - Add a Dockerfile and `docker-compose.yml` for the ResearchBoss FastAPI backend so any Docker-Compose-based deploy tool can upload and start it as-is. No web UI yet to include (Phase 10 not started). The Dockerfile's `pip install .` and exact `CMD` (single-worker `uvicorn`, required since session state is an in-memory dict — see the Dockerfile's own comment) were verified in a genuinely clean venv, serving `/health` successfully; the container build itself was not verified (no Docker available in this environment) — that step is documented as the reader's first move in `docs/DEPLOY.md`, not claimed as done.
- [x] **Done** - **API** - Add a persistent volume/bind-mount for the workspace root directory (`docker-compose.yml` bind-mounts `./data/workspaces` to `/data/workspaces`, matching `RESEARCHBOSS_WORKSPACE_ROOT`) so per-research workspace YAML, Markdown, SQLite, and document vault files survive container restarts and image updates, and are never baked into the deployed image.
- [x] **Done** - **Deterministic** - Add deployment setup notes for running `researchboss init` (via `POST /api/v1/projects/init`, already implemented) once per research project against the mounted workspace volume, so each research setup gets its own isolated workspace folder on the host (`docs/DEPLOY.md`).
- [ ] **API** - Actually deploy the ResearchBoss backend and web UI somewhere reachable. Not done: this is a per-person infrastructure step (a specific domain, host, and deploy tool), tracked in each deployer's own gitignored `docs/DEPLOY.personal.md`, not in this generic checklist.
...
- [x] **Done** - **API** - Add Zotero Web API account linking from the web UI, not just CLI discovery — new `POST`/`DELETE /api/v1/zotero/api/credentials` routes (and matching CLI `researchboss zotero api-link`/`api-unlink` commands, added for CLI/API parity) save/remove `ZOTERO_API_KEY`/`ZOTERO_USER_ID` in the workspace's local `.env`; the key is never echoed, logged, or returned by any route — only a `configured: true/false` confirmation. Added 2026-07-16 at Pedro's explicit request ("the user should be able to link their zotero account via the web interface, not only via CLI discovery"); this exceeds this phase's original scope, which assumed credentials would stay `.env`-only and pre-configured.
...
- [ ] **Deterministic** - Design a database-backend abstraction (likely a new `researchboss/engine/db_backends/` module used by `researchboss/engine/database.py`) so schema creation and sync operations can target SQLite and, optionally, MariaDB or PostgreSQL through one interface, without changing YAML/Markdown as the source of truth for any backend.
- [ ] **Deterministic** - Add `.env` config for the secondary backend: `RESEARCHBOSS_DB_BACKEND` (`sqlite` / `mariadb` / `postgres`, default `sqlite`) plus host/port/user/password/database vars for whichever is chosen. SQLite stays active whenever no secondary backend is configured — this must remain true with zero new required config, matching today's zero-setup behavior.
...
- [ ] **Deterministic** - CLI/API commands to add, list, tag, and search these notes/transcripts (e.g. `researchboss notes add/list/search`), stored as workspace Markdown/YAML like everything else, never sent anywhere until explicit AI opt-in.
...
- [ ] **Deterministic** - Add a CI/test check that runs the full deterministic (non-AI) test suite and a smoke pass over `researchboss doctor`/`researchboss status`/core CLI commands with every AI-related env var unset, to keep "the tool fully works with zero AI configured" an enforced guarantee rather than an assumption.
...
- [ ] **Deterministic** - New standalone entry point usable any time, not just during `init`: `researchboss rq wizard` — captures a free-text topic, then runs a fixed sequence of guiding questions (mixing open-answer and multiple-choice, reusing the existing `_prompt_numbered_choice` pattern already used elsewhere in `cli.py`) covering scope/context, the phenomenon or relationship of interest, question type (descriptive / comparative / causal / evaluative — a standard, deterministic research-methods taxonomy, not invented per-answer), what existing assumption is being tested, and what evidence would count as support vs. refutation.
- [ ] **Deterministic** - Extend the research question data model (`researchboss.engine.research_questions`) with explicit `hypothesis` (the claim being tested), `question_type`, and `proof_criteria`/`disproof_criteria` fields, populated deterministically from the wizard's answers — this operationalizes "default to a provable-or-disprovable claim" rather than an open-ended topic statement. Purely additive: existing fields (`question`, `subquestions`, `status`) and existing workflows (`rqs list/approve/reject/archive/check/assess`) keep working unchanged on RQs without these fields.
...
- [ ] **Deterministic** - `researchboss paper draft <rq-id>` (or an extension of the existing `research-question-brief` artefact type in `engine/artefact_creation.py`): a deterministic, AI-free paper *skeleton* assembled from existing artefact types (`research-question-brief`, `literature-review-matrix`, `claim-evidence-table`) into one document-vault target with a standard shape (hypothesis statement, background/lit review, evidence-for/evidence-against sections populated from the real claim ledger, a prove-or-disprove conclusion placeholder) — this is what "must work fully with zero AI configured" means for this feature: a genuinely useful scaffold from real workspace data, not empty prose.
- [ ] **AI** - `researchboss paper draft <rq-id> --ai`: an AI-assisted tier that fills the deterministic skeleton with actual drafted prose grounded in the workspace's accepted-source excerpts and claim ledger (same `build_safe_context` boundary every other AI route uses), aiming for genuinely submission-quality writing — but every generated paragraph must be traceable per AGENTS.md's "Core Rule: No Hallucinations" (Phase 27), and the command must explicitly flag/refuse sections the corpus doesn't support rather than inventing content to fill a gap.
...
From Pedro, 2026-07-16: this project's default shape is local-first, single-user (one shared `RESEARCHBOSS_API_USERNAME`/`RESEARCHBOSS_API_PASSWORD` pair per deployment, per Phase 9 — just upgraded from a password-only login to match a real account's login UX, but still one credential pair, not real accounts). If ResearchBoss is ever deployed commercially — one running instance serving multiple separate users or organizations — that needs real per-account isolation, which today's architecture does not provide at all. Not started; this is a significant scope/business decision (running a multi-user commercial service is a different product shape than the local-first tool this repo is built around today), not something to build speculatively ahead of Pedro deciding to actually pursue it.
...
- [ ] **Deterministic** - Per-account workspace isolation: each account can only see/access workspaces under their own namespace, extending the existing `RESEARCHBOSS_WORKSPACE_ROOT` containment model (today one shared root) to be per-account.
