...
- [ ] **API** - Add a popup overlay (modal) file preview for uploaded artefacts supporting PDF, image, text, and Markdown previews, dismissible by escape key, close control, or click-outside, without navigating away from the artefact list. Backend is now ready: `GET /api/v1/artefacts/uploads` (list) and `GET /api/v1/artefacts/uploads/{upload_id}/file` (raw bytes, `Content-Disposition: inline`) exist and are tested. Image preview is not reachable yet, though — `sources.ALLOWED_EXTENSIONS` (shared with the upload pipeline) has no image extensions, so that would need its own scoped decision (whether to add image extensions to the shared allow-list or give uploads their own). Remaining work here is purely the modal UI itself once Phase 10 picks a framework.
...
- [ ] **API** - Add an overlay review view for proposed cross-reference links and suggested renamed filenames, requiring explicit per-item approval before artefacts are cross-referenced into methodology or other artefact documents. Backend is now ready for both the cross-reference half (`GET /api/v1/artefacts/cross-reference`, `POST .../cross-reference/candidate-review`, `POST .../cross-reference/apply`) and citation plans (`POST /api/v1/citations/plan`, `POST .../plan/insertion-review`, `POST .../apply`) — both review-before-apply flows now have an API-reachable way to set a review_status without hand-editing YAML. Remaining work here is purely the overlay UI itself once Phase 10 picks a framework.
...
- [x] **Done** - **API** - Add a Dockerfile and `docker-compose.yml` for the ResearchBoss FastAPI backend so `../synology-site-deployer`'s existing `deploy` command can upload and start it as-is, with no changes to that project. No web UI yet to include (Phase 10 not started). The Dockerfile's `pip install .` and exact `CMD` (single-worker `uvicorn`, required since session state is an in-memory dict — see the Dockerfile's own comment) were verified in a genuinely clean venv, serving `/health` successfully; the container build itself was not verified (no Docker available in this environment) — that step is documented as the user's first move in `docs/DEPLOY.md`, not claimed as done.
- [x] **Done** - **API** - Add a persistent NAS volume/bind-mount for the workspace root directory (`docker-compose.yml` bind-mounts `./data/workspaces` to `/data/workspaces`, matching `RESEARCHBOSS_WORKSPACE_ROOT`) so per-research workspace YAML, Markdown, SQLite, and document vault files survive container restarts and image updates, and are never baked into the deployed image.
- [x] **Done** - **Deterministic** - Add NAS deployment setup notes for running `researchboss init` (via `POST /api/v1/projects/init`, already implemented) once per research project against the mounted workspace volume, so each research setup gets its own isolated workspace folder on the NAS (`docs/DEPLOY.md`).
