...
- [ ] **API** - Add `POST /api/v1/artefacts/cross-reference/apply` to write reviewed artefact cross-reference links. Deliberately not done: whether "write the link" means adding metadata to the artefact registry (like `linked_sources`) or literally inserting text into methodology/artefact document content (like citation apply does, needing per-format `.md`/`.docx`/`.pdf` handling) is a real product decision, not a mechanical follow-on from the read-only candidates route above.
