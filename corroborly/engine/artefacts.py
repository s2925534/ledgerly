        ...
        if artefact.get("id") == artefact_id:
            if artefact.get("paper_review_gate") == "requires_validate" and status in {"reviewed", "accepted"}:
                raise ValueError(
                    ...
                    "marked reviewed/accepted this way. Run `corroborly validate` against it, then "
                    "`corroborly paper clear-review-gate` (AGENTS.md Core Rule: a paper must never silently become "
...
def promote_ai_paper_draft(workspace: Path, artefact_id: str, applied_content_path: Path) -> dict[str, Any]:
    ...
    `corroborly validate` run) can clear it. A paper must never silently
...
def clear_paper_review_gate(workspace: Path, artefact_id: str) -> dict[str, Any]:
    ...
    real, up-to-date `corroborly validate <target>` run must already exist
    ...
    """
    from corroborly.engine.doc_validation import validation_report_path
    ...
    for artefact in artefacts:
        if artefact.get("id") == artefact_id:
            ...
            if not report_path.is_file():
                raise ValueError(
                    f"No validation report found for {target_path}. Run `corroborly validate` against it first."
                )
            if report_path.stat().st_mtime < target_path.stat().st_mtime:
                raise ValueError(
                    ...
                    "Run `corroborly validate` again after the most recent edit."
                )
