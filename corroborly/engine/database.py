...
from corroborly.engine.document_targets import PRIMARY_OUTPUT_ALIASES
...
DB_FILE_NAME = "corroborly.sqlite"
SCHEMA_VERSION = 1
...
def database_status(workspace: Path) -> DbCommandResult:
    path = database_path(workspace)
    if not path.exists():
        return DbCommandResult(path, {"status": "missing", "database": str(path), "repair": "run corroborly db init"})
    with _connect(path) as conn:
        _create_schema(conn)
        ...
        report = {
            ...
            "repair": "run corroborly db rebuild" if integrity != "ok" else None,
        }
    return DbCommandResult(path, report)
    ...
    if not path.exists():
        return DbCommandResult(
            path,
            {
                ...
                "query": query,
                "results": [],
                "hint": "Run `corroborly db sync` (or `db init` then `db sync`) to build the search index first.",
            },
        )
    fts_query = _fts_query_terms(query)
    if not fts_query:
...
def secondary_backend_status(workspace: Path) -> DbCommandResult:
    """Whether a secondary backend is configured (`CORROBORLY_DB_BACKEND` env
...
def activate_secondary_backend(workspace: Path) -> DbCommandResult:
    ...
    configured = configured_secondary_backend(workspace)
    if not configured:
        raise SecondaryBackendError("No secondary backend configured (CORROBORLY_DB_BACKEND is unset or 'sqlite').")
