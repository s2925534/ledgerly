...
from corroborly.engine.db_backends.base import SecondaryBackendCredentials, SecondaryBackendError
...
def configured_secondary_backend(workspace: Path | None = None) -> str | None:
    """`CORROBORLY_DB_BACKEND` if set to a real secondary backend name, else
    ...
    """
    value = (_env(workspace, "CORROBORLY_DB_BACKEND") or "sqlite").strip().lower()
    if value == "sqlite" or not value:
        return None
    if value not in SECONDARY_BACKENDS:
        allowed = ", ".join(sorted({"sqlite", *SECONDARY_BACKENDS}))
        raise SecondaryBackendError(f"Invalid CORROBORLY_DB_BACKEND: {value!r}. Expected one of: {allowed}")
    return value


def secondary_backend_credentials(backend: str, workspace: Path | None = None) -> SecondaryBackendCredentials:
    prefix = backend.upper()
    host = _env(workspace, f"CORROBORLY_{prefix}_HOST")
    user = _env(workspace, f"CORROBORLY_{prefix}_USER")
    password = _env(workspace, f"CORROBORLY_{prefix}_PASSWORD") or ""
    database = _env(workspace, f"CORROBORLY_{prefix}_DATABASE")
    port_raw = _env(workspace, f"CORROBORLY_{prefix}_PORT")
    missing = [
        name
    ...
    if missing:
        raise SecondaryBackendError(
            f"Missing required config for CORROBORLY_DB_BACKEND={backend}: "
            + ", ".join(f"CORROBORLY_{prefix}_{name}" for name in missing)
        )
    try:
        port = int(port_raw) if port_raw else DEFAULT_PORTS[backend]
    except ValueError as exc:
        raise SecondaryBackendError(f"Invalid CORROBORLY_{prefix}_PORT: {port_raw!r}") from exc
