...
async function promotePaperDraftAi() {
  ...
  try {
    ...
    messageEl.textContent = "Promoted. Run `corroborly validate` against it, then clear the review gate.";
    await refreshArtefacts();
  } catch (err) {
    messageEl.textContent = err.message;
    messageEl.classList.add("error");
...
const LAST_WORKSPACE_KEY = "corroborly:lastWorkspace";
...
// routes the CLI's `corroborly zotero` commands use. Never writes inside
