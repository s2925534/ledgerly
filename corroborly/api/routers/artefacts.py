...
from corroborly.engine.ai import OpenAiError, openai_credentials
...
from corroborly.engine.sources import ALLOWED_EXTENSIONS
from corroborly.engine.vault import intake_uploaded_artefact_batch, list_uploaded_artefacts, resolve_uploaded_artefact_file
...
# Preview-relevant subset of corroborly.engine.sources.ALLOWED_EXTENSIONS.
...
def artefacts_paper_promote_ai_draft(
    ...
    from corroborly.engine.ai_edit_sessions import get_ai_edit_session
    from corroborly.engine.artefact_creation import SUPPORTED_ARTEFACT_TYPES
...
def artefacts_upload_limits() -> dict[str, Any]:
    ...
    (CORROBORLY_UPLOAD_MAX_FILES/MAX_FILE_SIZE_MB, plus the shared
    ...
    return ok(
        {
            "max_files": _int_env("CORROBORLY_UPLOAD_MAX_FILES", DEFAULT_UPLOAD_MAX_FILES),
            "max_file_size_mb": _float_env("CORROBORLY_UPLOAD_MAX_FILE_SIZE_MB", DEFAULT_UPLOAD_MAX_FILE_SIZE_MB),
...
async def artefacts_upload(
    ...
    CORROBORLY_UPLOAD_MAX_FILES, rather than silently processing only
    some of them. Each file is capped at CORROBORLY_UPLOAD_MAX_FILE_SIZE_MB
    ...
    max_size_bytes = int(_float_env("CORROBORLY_UPLOAD_MAX_FILE_SIZE_MB", DEFAULT_UPLOAD_MAX_FILE_SIZE_MB) * 1024 * 1024)
    temp_dir = Path(tempfile.mkdtemp(prefix="corroborly-upload-"))
    try:
        temp_paths = []
