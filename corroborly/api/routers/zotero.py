...
from corroborly.engine.sources import iter_source_files
...
def zotero_local_select_collections(
    ...
    """Configure selected local Zotero collections for future scans (mirrors `corroborly zotero select-collections`)."""
...
def zotero_local_use_entire_library(workspace: Path = Depends(resolve_workspace)) -> dict[str, Any]:
    """Configure local Zotero scans to use the entire storage library (mirrors `corroborly zotero use-entire-library`)."""
